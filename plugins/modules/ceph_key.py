...
try:
    from ansible_collections.ceph.automation.plugins.module_utils.ceph_key_common import (
        exec_commands,
    )
except ImportError:
...
def generate_ceph_authtool_cmd(
    module: "AnsibleModule", cluster, name, secret, caps, dest, container_image=None
) -> list:  # noqa: E501
    """
...
def create_key(
    ...
    cmd_list.append(
        generate_ceph_authtool_cmd(
            module, cluster, name, secret, caps, dest, container_image
        )
    )

    if import_key or user != "client.admin":
        base_cmd = build_base_cmd_shell(module)
        base_cmd.extend(["auth"] + args)
        cmd_list.extend(base_cmd)
    return cmd_list


def delete_key(
    module: "AnsibleModule", cluster, user, user_key, name, container_image=None
):
    """
    Delete a CephX key
    """
...
def get_key(
    module: "AnsibleModule", cluster, user, user_key, name, dest, container_image=None
):
    """
    ...
    base_cmd = build_base_cmd_shell(module)
    base_cmd.extend(["ceph", "auth"])
    cmd_list.append(base_cmd + args)
...
def run_module():
    module_args = dict(
        ...
        import_key=dict(type="bool", required=False, default=True, no_log=False),
    ...
    if state in ["present", "update"]:
        ...
        if rc == 0:
            rc, cmd, out, err = exec_commands(
                module,
                delete_key(module, cluster, user, user_key_path, name, container_image),
            )  # noqa: E501
            changed = True
        else:
            rc = 0
